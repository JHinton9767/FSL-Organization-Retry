...
def term_from_row(row: dict[str, str]) -> tuple[str, str, int]:
    term_code, term_label, term_sort = parse_term(row.get("term_code", ""))
    label_code, label_label, label_sort = parse_term(row.get("term_label", ""))
    ...
    return label_code, label_label, label_sort
...
def build_chapter_semester_exports(roster_path: Path, output_dir: Path) -> ChapterSemesterExportResult:
    ...
    write_csv(matrix_path, matrix_columns, matrix)
    write_csv(lifecycle_path, LIFECYCLE_COLUMNS, lifecycle)
